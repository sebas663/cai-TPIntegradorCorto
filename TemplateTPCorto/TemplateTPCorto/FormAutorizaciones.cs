...
namespace TemplateTPCorto
{
    public partial class FormAutorizaciones : UserControl
    {
        ...
        private void btnAutorizarCambioCredencial_Click(object sender, EventArgs e)
        {
            ...
            dgwAutorizarOperaciones.DataSource = loginNegocio.ObtenerOperacionesCambioCredencial();
            AgregarColumnaSeleccion(dgwAutorizarOperaciones);
        }

        private void btnAutorizarCambioPersona_Click(object sender, EventArgs e)
        {
            esCambioCredencial = false;
            LoginNegocio loginNegocio = new LoginNegocio();
            dgwAutorizarOperaciones.Columns.Clear();
            dgwAutorizarOperaciones.DataSource = loginNegocio.ObtenerOperacionesCambioPersona();
            AgregarColumnaSeleccion(dgwAutorizarOperaciones);
        }
        ...
        private void btnAutorizar_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                if (esCambioCredencial)
                {
                    ...
                    loginNegocio.AutorizarOperacionesCambioCredencial(operaciones);
                }
                else
                {
                    List<OperacionCambioPersona> operaciones = ObtenerOperacionesCambioDatosPersonasSeleccionadas();
                    loginNegocio.AutorizarOperacionesCambioPersona(operaciones);
                }
